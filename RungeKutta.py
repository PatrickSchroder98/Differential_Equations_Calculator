class RungeKutta:
    ...
    def RungeKutta3(self, F, X, Y1, n, h):
        for i in range(n):
            ...
            X.append(X[i] + h)
            Y1.append(Y1[i] + ((2.0 / 9.0) * k1) + ((1.0 / 3.0) * k2) + ((4.0 / 9.0) * k3))
    ...
    def RungeKutta5(self, F, X, Y1, n, h):
        ...
        for i in range(n):
            ...
            X.append(X[i] + h)
            Y1.append(Y1[i] + (1.0 / 90.0) * (7.0 * k1 + 32.0 * k2 + 12.0 * k4 + 32.0 * k5 + 7.0 * k6))
    ...
    def RungeKutta6(self, F, X, Y1, n, h):
        ...
        for i in range(n):
            ...
            X.append(X[i] + h)
            Y1.append(Y1[i] + (1.0 / 840.0) * (41.0 * (k1 + k8) + 216.0 * (k3 + k7) + 27.0 * (k4 + k6) + 272.0 * k5))
